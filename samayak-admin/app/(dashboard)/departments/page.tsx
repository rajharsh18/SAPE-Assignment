...
export default function DepartmentsPage() {
  ...
  const handleSaveBranch = async () => {
    const res = await fetch(
      `/api/departments/${branchForm.departmentId}/branches`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      <DataTable
        ...
        columns={[
          ...
          },
          {
            key: "code",
            header: "Code",
            render: (d) => <Badge variant="brand">{d.code}</Badge>,
          },
          {
            key: "branches",
            header: "Branches",
            align: "center",
            render: (d) => d._count.branches,
          },
          {
            key: "rooms",
            header: "Rooms",
            align: "center",
            render: (d) => d._count.rooms,
          },
          {
            key: "courses",
            header: "Courses",
            align: "center",
            render: (d) => d._count.courses,
          },
          {
            key: "faculty",
            header: "Faculty",
            align: "center",
            render: (d) => d._count.faculty,
          },
          {
            key: "actions",
            header: "Actions",
      ...
      >
        <div className="space-y-4">
          <Select
            ...
            onChange={(e) =>
              setBranchForm({ ...branchForm, departmentId: e.target.value })
            }
          >
            {departments.map((d) => (
          ...
          <div className="grid grid-cols-2 gap-3">
            <Input
              label="Semester"
              type="number"
              value={branchForm.semester}
              onChange={(e) =>
                setBranchForm({ ...branchForm, semester: e.target.value })
              }
            />
            <Input
              label="Section"
              value={branchForm.section}
              onChange={(e) =>
                setBranchForm({ ...branchForm, section: e.target.value })
              }
              placeholder="A"
            />
          </div>
          <Button onClick={handleSaveBranch} className="w-full justify-center">
            Create Branch
          </Button>
        </div>
      </Modal>
      ...
      >
        <div className="space-y-3">
          <div className="flex gap-2 p-3 rounded-[14px] bg-warning/10 text-warning text-sm">
            ...
            <p>
              <strong>{deleteWarning?.dept.name}</strong> has dependent records
              that must be removed first.
            </p>
          </div>
          <div className="grid grid-cols-2 gap-2 text-sm">
            {deleteWarning &&
              Object.entries(deleteWarning.deps)
                .filter(([, v]) => v > 0)
                .map(([key, value]) => (
